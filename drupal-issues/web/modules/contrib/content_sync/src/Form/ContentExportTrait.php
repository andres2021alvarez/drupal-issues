...
trait ContentExportTrait {
  ...
  /**
   * Generates the archive batch file.
   *
   ...
   *
   * @return array
   *   The exported entities batch array
   */
        ...
        if (!isset($context['exported'][$name])) {
          ...
          if (isset($serializer_context['export_type'])) {
            if ($serializer_context['export_type'] == 'snapshot') {
              ...
              $activeStorage->csWrite($name, Yaml::decode($exported_entity), $entity_type . '.' . $bundle);
            }
            else {
              ...
              $activeStorage = new ContentDatabaseStorage(\Drupal::database(), 'cs_db_snapshot');
              $exported_entity_snapshoot = $activeStorage->csRead($name);
              ...
              if ($serializer_context['export_type'] == 'folder') {
                ...
                \Drupal::service('file_system')->saveData($exported_entity, $destination, FileSystemInterface::EXISTS_REPLACE);
              }
  ...
  public function generateSiteUuidFile($serializer_context, &$context) {
    ...
    if (isset($serializer_context['export_type'])) {
      ...
      elseif ($serializer_context['export_type'] == 'folder') {
        ...
        $destination = $path . "/$name.yml";
        \Drupal::service('file_system')->prepareDirectory($path, FileSystemInterface::CREATE_DIRECTORY);
        \Drupal::service('file_system')->saveData(Yaml::encode($entity), $destination, FileSystemInterface::EXISTS_REPLACE);
      }
    }
    $context['message'] = $name;
    $context['results'][] = $name;
  ...
  /**
   * The content exporter interface.
   *
